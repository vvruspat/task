...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    return NextResponse.json(await result.api.getGoogleDriveFolderAssignment(input));
  } catch (error: unknown) {
    return taskApiErrorResponse(error, "Unable to load the Google Drive folder.");
  }
}
...
export async function PUT(request: Request): Promise<NextResponse> {
  ...
  if (input === null || value["folderId"].trim().length === 0) return invalidTargetResponse();
  const result = createServerTaskApi(request);
  if (result.response !== undefined) return result.response;
  try {
    return NextResponse.json(
      await result.api.selectGoogleDriveFolder({
        ...input,
        body: { folderId: value["folderId"].trim() },
      }),
    );
  } catch (error: unknown) {
    return taskApiErrorResponse(error, "Unable to assign the Google Drive folder.");
  }
}
...
function invalidTargetResponse(): NextResponse {
  return NextResponse.json(
    { error: "Google Drive folder assignment input is invalid." },
    { status: 400 },
  );
}
