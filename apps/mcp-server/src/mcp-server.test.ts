...
test("registerSummaryTools registers summary tools", async () => {
  const toolCalls: RegisteredToolCall[] = [];
  ...
  assert.deepEqual(JSON.parse(readTextResult(summaryResult)), {
    task: {
      id: rootTaskId,
      workspaceId,
      projectId,
...
test("registerTaskTools registers task tools", async () => {
  ...
  const addSubtasksCalls: AddTaskSubtasksRequest[] = [];
  ...
  registerTaskTools(registrar, {
    ...
    addSubtasks: async (input: unknown): Promise<TaskDetailResponse[]> => {
      if (!isUnknownRecord(input)) {
        throw new Error("Expected task add subtasks input.");
      }
      addSubtasksCalls.push({
        workspaceId: readString(input, "workspaceId"),
        projectId: readString(input, "projectId"),
        taskId: readString(input, "taskId"),
        userId: readString(input, "userId"),
    ...
    update: async (input: unknown): Promise<TaskDetailResponse> => {
      if (!isUnknownRecord(input)) {
        throw new Error("Expected task update input.");
      }
      updateCalls.push({
        workspaceId: readString(input, "workspaceId"),
        projectId: readString(input, "projectId"),
  ...
  assert.equal(toolCalls[4]?.config.title, "Move task");
  assert.equal(toolCalls[5]?.config.title, "Set task assignee");
  assert.equal(toolCalls[6]?.config.title, "Assign task");
  assert.equal(toolCalls[7]?.config.title, "Set task due date");
  assert.equal(toolCalls[8]?.config.title, "Get task");
  assert.equal(toolCalls[9]?.config.title, "Archive task");
  assert.equal(toolCalls[10]?.config.title, "Search tasks");
  ...
  assert.deepEqual(JSON.parse(readTextResult(updateResult)), taskResponse);
  assert.deepEqual(updateCalls, [
    {
      workspaceId,
      projectId,
  ...
  const addSubtasksCall = toolCalls[3];
  assert.ok(addSubtasksCall !== undefined);
  const addSubtasksResult = await addSubtasksCall.callback({
    workspaceId,
    projectId,
    taskId: rootTaskId,
    userId,
    subtasks: [{ title: "Record drums" }],
  });

  assert.deepEqual(JSON.parse(readTextResult(addSubtasksResult)), [taskResponse]);
  assert.deepEqual(addSubtasksCalls, [
    {
      workspaceId,
      projectId,
      taskId: rootTaskId,
  ...
  const assigneeCall = toolCalls[5];
  ...
  const assignCall = toolCalls[6];
  assert.ok(assignCall !== undefined);
  const assignResult = await assignCall.callback({
    workspaceId,
    projectId,
    taskId: rootTaskId,
    userId,
  ...
  assert.deepEqual(JSON.parse(readTextResult(assignResult)), taskResponse);
...
function createBackendClientStub(): TaskBackendClient {
  return {
    ...
    addTaskSubtasks: async (): Promise<TaskDetailResponse[]> => [taskResponse],
