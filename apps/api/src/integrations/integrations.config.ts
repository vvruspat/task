...
export type IntegrationsEnvironment = {
  ...
  YANDEX_DISK_CLIENT_ID?: string;
  YANDEX_DISK_CLIENT_SECRET?: string;
  YANDEX_DISK_REDIRECT_URI?: string;
};
...
export type YandexDiskOAuthConfig = {
  clientId: string;
  clientSecret: string;
  redirectUri: string;
};
...
export type IntegrationsConfig = {
  ...
  yandexDisk: YandexDiskOAuthConfig | null;
};
...
export function parseIntegrationsConfig(environment: IntegrationsEnvironment): IntegrationsConfig {
  ...
  const yandexDisk = parseYandexDiskOAuthConfig(environment);
...
function parseAttachmentContentConfig(
  ...
  return { storageRoot: normalizedRoot };
}
...
function parseYandexDiskOAuthConfig(
  environment: IntegrationsEnvironment,
): YandexDiskOAuthConfig | null {
  const clientId = environment.YANDEX_DISK_CLIENT_ID;
  const clientSecret = environment.YANDEX_DISK_CLIENT_SECRET;
  const redirectUri = environment.YANDEX_DISK_REDIRECT_URI;
  if (clientId === undefined && clientSecret === undefined && redirectUri === undefined)
    return null;
  assertTrimmedValue("YANDEX_DISK_CLIENT_ID", clientId);
  assertTrimmedValue("YANDEX_DISK_CLIENT_SECRET", clientSecret);
  assertTrimmedValue("YANDEX_DISK_REDIRECT_URI", redirectUri);
  if (clientId === undefined || clientSecret === undefined || redirectUri === undefined) {
    throw new Error("Yandex Disk OAuth configuration validation did not narrow its values.");
  }
  let parsedRedirectUri: URL;
  try {
    parsedRedirectUri = new URL(redirectUri);
