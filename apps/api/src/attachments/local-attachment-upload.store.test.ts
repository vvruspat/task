...
test("local attachment upload store writes private content under a generated task key", async (context) => {
  ...
  const store = new LocalAttachmentUploadStore({ maxBytes: 10, storageRoot: rootPath });
...
test("local attachment upload store requires configured storage and enforces its byte limit", async () => {
  await assert.rejects(
    new LocalAttachmentUploadStore({ maxBytes: 10, storageRoot: null }).store({
      bytes: Buffer.from("hello"),
      taskId,
      workspaceId,
    }),
  ...
  const rootPath = await mkdtemp(join(tmpdir(), "task-file-upload-limit-"));
  try {
    await assert.rejects(
      new LocalAttachmentUploadStore({ maxBytes: 4, storageRoot: rootPath }).store({
        bytes: Buffer.from("hello"),
        taskId,
        workspaceId,
      }),
